...
def render():
    ...
    # ===Delete Book===
    delete_books = "DElETE FROM issued WHERE book_issued = %s"
    data_books = (book,)
    # ===Update Quantity===
    update_quantity = "UPDATE books SET quantity = %s where title = %s"
    d_b = (got, book,)
    try:
        cur.execute(delete_books, data_books)
        con.commit()
        cur.execute(update_quantity, d_b)
        con.commit()
        messagebox.showinfo('Success', "Book returned successfully")
    except (pymysql.Error, pymysql.Warning):
        messagebox.showerror("Error", "Can't update data into Database")
...
def return_book():
    ...
    heading_label = Label(heading_frame1, text="Return Book", bg='#9d8764', fg='light gray', font=('Calibri bold', 24))
